...
do {
  ...
  if (isMaster) {
    ...
    config.httpScripts.forEach(function(script) {
      business =  require('./scripts/' + script);
    ...
    var statistic = {
      ...
      'toruk.tanx.com': 1,
      'ecpm.tanx.com': 1,
      'apollon.t.taobao.com': 1,
      'tmatch.simba.taobao.com': 1,
      'pos.baidu.com': 1,
      'item.yhd.com': 1,
      'suggestion.baidu.com': 1,
      'cb.baidu.com': 1,
      'open.show.qq.com': 1,
      'www.baidu.com': 1,
      'suggest.taobao.com': 1,
    };

    loggerApp.info('开启线程监听msgId为 ' + evn.msgId + ' msgType为 ' + evn.msgType + ' 队列' );
    var bypass = require('./ipc-bypass/bypass')(parseInt(evn.msgId, 10));
    bypass.listen(pagesize, parseInt(evn.msgType, 10));
    bypass.on('data', function(error, data) {
        console.log(data);
        delete data;
        // if (error) { loggerApp.fatal(error);}
        // if (data) {
        //   var script = scripts[data.host];
        //   if (script) {
        //     var httpData = script.getData(data);
        //     if (httpData) {
        //       data.info.httpData = httpData;
        //       sender.send(data.info);
        //     }
        //   }
        //   if (statistic[data.host]) {
        //     util.cacheIncr(data.host);
        //   }
        // }
    });
