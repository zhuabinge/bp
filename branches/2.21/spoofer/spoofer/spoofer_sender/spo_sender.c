                                 ...
                                 libnet_t *handle, u_char  *playload, uint playload_size)
{
    ...
    t = libnet_build_ipv4(
                ...
                SPO_IP_TOL,                                             /* ip tol */
                18002,//(u_short) libnet_get_prand(LIBNET_PRu16),               /* ip id */
                hjk_info->ip_off,                               /* can fragment */
                ttl,                                                    /* ip ttl */
...
static SPO_RET_STATUS
_spo_send_http_rsp_pkt(spo_hp_hjk_t *hjk_info,
                                 libnet_t *handle, u_char  *playload, uint playload_size)
{
    libnet_ptag_t t;
    int ret = SPO_FAILURE;
    ...
    t = libnet_build_tcp(
                ...
                hjk_info->tcp_resp_seq,                                 /* tcp seq */
                hjk_info->tcp_resp_Ack,                                 /* tcp ACK */
                hjk_info->tcp_resp_flg,                                 /* tcp flags */
                SPO_TCP_WIN_SIZE,                                       /* win size */
                SPO_NO_SUM_CHECK,                                       /* check sum */
                SPO_NO_TCP_URG,                                         /* tcp ueg targe */
                LIBNET_TCP_H + playload_size,                           /* tcp total size */
...
static SPO_RET_VALUE __spo_snd_http_pkt(libnet_t *handler, spo_hp_hjk_t *hjk_info, u_char *data, size_t size)
{
    size_t off = 0;

    if (spo_unlikely(!handler || !hjk_info || !data || size == 0)) return SPO_FAILURE;

    hjk_info->tcp_resp_flg =  TH_PUSH | TH_ACK;

    while (off < size) {
        if (off + SPO_MTU >= size) {
            hjk_info->tcp_resp_flg =  hjk_info->tcp_resp_flg | TH_FIN;
            if ((_spo_send_http_rsp_pkt(hjk_info, handler, data + off, size - off)) == SPO_FAILURE) return SPO_FAILURE;
            return size;
        }

        if ((_spo_send_http_rsp_pkt(hjk_info, handler, data + off, SPO_MTU)) == SPO_FAILURE) return SPO_FAILURE;

        off += SPO_MTU;
        hjk_info->tcp_resp_seq = hjk_info->tcp_resp_seq + SPO_MTU;
        hjk_info->ip_id++;
    }
...
static SPO_RET_STATUS spo_snd_http_pkt(spo_msg_t *msg)
{
    ...
    if (hjk_info->vlan_targe == SPO_RUNNING_IN_VLAN) {      /* running in the vlan env */
        __spo_snd_http_pkt(handle_lnk, hjk_info, p, p_size);
        ...
        __spo_snd_http_pkt(handle_raw, hjk_info, p, p_size);
