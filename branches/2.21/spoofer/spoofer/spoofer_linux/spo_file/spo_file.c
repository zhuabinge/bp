...
/*  */
ssize_t spo_write2file(const char *file, const void *buf, size_t size)
{
    int fd = 0;
    ssize_t s = 0;

    if (buf == NULL || size <= 0 || file == NULL) return 0;

    if ((fd = spo_open(file, O_CREAT | O_RDWR, 0666)) == SPO_FAILURE) {
#if SPO_DEBUG
...
SPO_RET_STATUS spo_folder_exist(const char *folder_path)
{
    struct stat info;

    memset(&info, '\0', sizeof(struct stat));

    if (stat(folder_path, &info) == SPO_FAILURE) return SPO_FAILURE;

    if (S_ISDIR(info.st_mode)) return SPO_OK;
