...
static SPO_RET_STATUS
spo_calc_data_len(spo_proc_node_t *p_node, spo_bld_pkt_t *bld_pkt, u_char **ch, u_char *hd_end);
...
static char *spo_header_end = (char *) "\r\n\r\n";
...
spo_calc_data_len(spo_proc_node_t *p_node, spo_bld_pkt_t *bld_pkt, u_char **ch, u_char *hd_end)
{
    ...
    sprintf(con_len, "Content-Length: %d\r\n", (uint) ((p + bld_pkt->h_header_s + bld_pkt->h_data_s) - hd_end));

    memcpy(hd_start, p, SPO_STATUS_LINE_LEN);

    p -= (strlen(con_len) - SPO_STATUS_LINE_LEN);
    memcpy(p, con_len, strlen(con_len));

    p -= SPO_STATUS_LINE_LEN;
    memcpy(p, hd_start, SPO_STATUS_LINE_LEN);

    bld_pkt->http_start = p - (u_char *) (p_node->snd_pkt);
    bld_pkt->h_header_s = bld_pkt->h_header_s + strlen(con_len);
...
static SPO_RET_STATUS
spo_do_build_data(spo_hp_data_t *data, spo_str_t *infos, spo_proc_node_t *p_node)
{
    spo_hp_data_info_t *tag_info = data->tail;
    u_char *data_tail = data->data.data + data->data_start;
    ...
    while (tag_info != NULL) {
        ...
        if(tag_info->type == 7) {
            hd_end = p;
            p -= tag_info->len;
            memcpy(p, spo_header_end, tag_info->len);
        } else {
    ...
    return spo_calc_data_len(p_node, bld_pkt, &p, hd_end);
}
...
static SPO_RET_BOOLEN spo_hp_pcre_match(pcre *pr, spo_str_t *info)
{
    ...
    ret = pcre_exec(pr, NULL, (char *) info->data, info->len, 0, 0, ovector, SPO_OVECCOUNT);
    if (ret == PCRE_ERROR_NOMATCH) return SPO_FALSE;
...
spo_do_hijacking_http_info(spo_hp_hjk_t *hjk_info, spo_sniff_ip_t *ip, spo_sniff_tcp_t *tcp)
{
    ...
    hjk_info->ip_id = (u_short) libnet_get_prand(LIBNET_PRu16);
...
static void spo_do_hp_spoofer(spo_proc_node_t *p_node)
{
    ...
    while (SPO_TRUE) {
        ...
        /* full hjk info, snd msg */
        spo_spoofer_sender_info((u_char *) pkt->packet, p_node->snd_pkt, snd_msgs[counter]);
...
/* for per domain, snd the statis info */
