...
static SPO_RET_STATUS spo_analysts_cfg_blk(spo_cfg_t *cfg, spo_cfg_stat *status);
static SPO_RET_STATUS spo_do_analysts_cfg_blk(spo_cfg_t *cfg, char *line, const char *delim);
...
static SPO_RET_STATUS spo_init_cfg_snif_analy_msgids(spo_info_t *info, const char *p);
...
spo_analy_info_t *spo_create_analy_info()
{
    ...
    if ((analy_info = spo_calloc(sizeof(spo_analy_info_t))) == NULL) return NULL;

    analy_info->hp_msgid    = NULL;
    analy_info->info        = NULL;
    analy_info->next        = NULL;
    analy_info->dev         = NULL;
...
{
    if (analy_info == NULL) return SPO_OK;
    ...
    while (p != NULL) {
        analy_info = analy_info->next;
        if (p->hp_msgid != NULL) spo_free(p->hp_msgid);
        spo_free(p);
        p = analy_info;
    }
...
static SPO_RET_STATUS spo_init_analy_info_header(spo_analy_info_header_t *header)
{
...
spo_analy_info_header_t *spo_create_analy_info_header()
{
    spo_analy_info_header_t *header = NULL;

    if ((header = spo_calloc(sizeof(spo_analy_info_header_t))) == NULL) return NULL;

    spo_init_analy_info_header(header);

    return header;
}


SPO_RET_STATUS spo_destorty_analy_info_header(spo_analy_info_header_t *header)
{
    if (header == NULL) return SPO_OK;

    if (header->infos != NULL) spo_destory_analy_info(header->infos);
...
static SPO_RET_STATUS spo_analysis_global_instr(spo_cfg_t *cfg, char *line, char *delim)
{
    ...
    if (strcmp(p, "pf_watermark") == 0) {
        if ((p = strtok(NULL, "")) != NULL) {
            cfg->global->pf_watermark = atoi(p);
            return SPO_OK;
        }
        return SPO_FAILURE;
    }

    if (strcmp(p, "analysts_cache") == 0) {
        if ((p = strtok(NULL, "")) != NULL) {
            cfg->global->analysts_cache = atoi(p) * 1024 * 1024;
            return SPO_OK;
        }
        return SPO_FAILURE;
    }
...
static SPO_RET_STATUS spo_init_cfg_anly_hp_msgids(spo_analy_info_t *info, const char *p)
{
    static int max_amount = SPO_DEF_MSGID_AMOUNT;

    if (info->hp_msgid == NULL)
        if ((info->hp_msgid = spo_create_cfg_msgids(SPO_DEF_MSGID_AMOUNT)) == NULL)
            return SPO_FAILURE;

    if (info->hp_msgid[0] >= max_amount) {
        void *p = NULL;

        p = realloc(info->hp_msgid, (info->hp_msgid[0] + SPO_INC_MSGID_AMOUNT) * sizeof(int));
        if (p == NULL) return SPO_FAILURE;
    ...
    info->hp_msgid[0]++;
    info->hp_msgid[info->hp_msgid[0]] = atoi(p);
...
static SPO_RET_STATUS spo_analysis_sniff_cfg(spo_cfg_t *cfg, char *line, char *delim)
{
    ...
    if (memcmp(key, "analy_msgid", strlen("analy_msgid")) == 0) {
        return spo_init_cfg_snif_analy_msgids(info, p);
    }
...
static SPO_RET_STATUS spo_do_analysts_cfg_blk(spo_cfg_t *cfg, char *line, const char *delim)
{
    size_t len = 0;
    char *p = NULL;
    spo_analy_info_t *info = NULL;
    char key[32] = {'\0'};

    info = cfg->global->analy_hd->tail;
    ...
    if (memcmp(key, "dev", strlen("dev")) == 0) {
        if ((info->dev = spo_calloc(len + 1)) == NULL) return SPO_FAILURE;
        memcpy(info->dev, p, len);
        return SPO_OK;
    }
    ...
    if (memcmp(key, "http_msgid", strlen("http_msgid")) == 0) {
        return spo_init_cfg_anly_hp_msgids(info, p);
    }
...
static SPO_RET_STATUS spo_analysts_cfg_blk(spo_cfg_t *cfg, spo_cfg_stat *status)
{
    ...
    if (cfg->global->analy_hd == NULL) {
        if ((header = spo_create_analy_info_header()) == NULL) return SPO_FAILURE;
        cfg->global->analy_hd = header;
    }

    header = cfg->global->analy_hd;

    if ((info = spo_create_analy_info()) == NULL) return SPO_FAILURE;

    if (header->infos == NULL) {
        header->infos = info;
        header->tail = info;
    }else {
        header->tail->next = info;
        header->tail = info;
    }

    header->amt++;

    *status = SPO_IN_ANALYS;
...
static SPO_RET_STATUS spo_analysis_cfg(char *line, spo_cfg_t *cfg, spo_cfg_stat *status)
{
    ...
    if (*status == SPO_IN_ANALYS) {
        return spo_do_analysts_cfg_blk(cfg, line, " ");
    }
    ...
    if (memcmp(line, "<spo_analysts>", strlen("<spo_analysts>")) == 0) {
        return spo_analysts_cfg_blk(cfg, status);
    }
...
static SPO_RET_STATUS spo_cp_http_data(spo_hp_data_t *hp_data, spo_str_t *data_cp, spo_str_t *data, size_t size)
{
    ...
    for (i = 0; i < data->len; i++) {
        if (*p == 0x2f && *(p + 1) == 0x2a && *(p + 2) == 0x2a && *(p + 3) == 0x2f) {
            hp_data->data_start = p - data->data;
...
static SPO_RET_STATUS spo_hp_hd_tag(spo_hp_data_t *hp_data, spo_hp_data_info_t *info, u_char **tag)
{
    info->len = 4;
    info->type = 7;
    (*tag) += 4;

    return spo_hp_data_info_insert(hp_data, info);
}
...
static SPO_RET_STATUS spo_http_data_pat_find(spo_hp_data_t *hp_data, u_char **tag)
{
    ...
    if (memcmp(ch, SPO_HEADER_END, SPO_INFO_TAG_LEN) == 0) {
        return spo_hp_hd_tag(hp_data, info, tag);
    }
...
static SPO_RET_STATUS spo_hp_data_tag_match(spo_hp_data_t *hp_data)
{
    ...
    while (i < hp_data->data.len) {
        ...
        if ((*ch == '$') || (*ch == 0x0d && *(ch + 1) == 0x0a && *(ch + 2) == 0x0d && *(ch + 3) == 0x0a)) {
                  ...
                  int (*comp_func) (void *, void *), int (*spo_free_key_func) (void *))
{
    ...
    if (spo_folder_exist(p_name) == SPO_FAILURE) {
        printf("this folder is not exist\n");
        return NULL;
    }
...
static SPO_RET_STATUS spo_check_g_cfg(spo_cfg_g_t *g)
{
    ...
    if (g->pf_watermark > 256)
        g->pf_watermark = 256;
    else if (g->pf_watermark <= 0) g->pf_watermark = 1;

    if (g->analysts_cache < 16 * 1024 * 1024) g->analysts_cache = 16 * 1024 * 1024;
        ...
        if (memcmp(info->type, "sniffer", strlen("sniffer")) == 0) {
            ...
            if (memcmp(info->lib, "pf", 2) == 0) {
                if (info->driection == 0) {
                    ...
                    spo_snd_log_msg(sys_log, log, SPO_MAIN, SPO_LOG_LEVEL_MSG, getpid());
                    info->driection = 1;        /* defluat is rx */
                }
            }
