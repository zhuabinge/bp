...
var onError = function(error) {
  log.getLogger('nogfw').fatal(require('util').format(error.stack), true);
  if (isMaster) {
    log.getLogger('nogfw').fatal('服务进程异常', daemonize ? false : true);
  }
  process.exit(1);
};
process.on('uncaughtException', function(error) {
  log.getLogger('nogfw').fatal(require('util').format(error.stack));
  onError(error);
});
...
do {
  ...
  log.setLogger('nogfw', logPath + '/server.log');
  // 检查服务配置
  if (config.service === undefined || !util.isArray(config.service) || !config.service.length) {
    if (isMaster) {
      log.getLogger('nogfw').fatal('没有配置任何服务', true);
      log.getLogger('nogfw').fatal('服务进程异常', true);
    }
    break;
  }
  if (isMaster) { // 主进程
    ...
    if (fs.existsSync(pid)) {
      log.getLogger('nogfw').notice('服务进程已存在', true);
      break;
    }
    if (daemonize) {
      // 后台运行
    ...
    process.on('exit', function() {
      ...
      log.getLogger('nogfw').info('服务进程已终止', daemonize ? false : true);
    });
    // 启动子进程
    ...
    for (var i = 0; i < workerProcesses; ++i) {
      workers.push(cluster.fork({worker_id: i}));
    }
    log.getLogger('nogfw').info('服务进程已启动, 子进程数 ' + workerProcesses, daemonize ? false : true);
  } else {  // 子进程
    var services = {};
    process.on('exit', function() {
      // 服务卸载函数
    ...
    domainLoad.on('error', function(error) {
      log.getLogger('nogfw').fatal(error);
      log.getLogger('nogfw').fatal('子进程异常终止', true);
      process.exit(1);
    });
    domainLoad.run(function() {
      ...
      for (var i in config.service) {
        ...
        try {
          ...
          log.getLogger('nogfw').fatal('[' + serviceName + '] ' + error);
          loaded = false;
          break;
        }
      }
    ...
    domainRest.run(function() {
      ...
      server.listen(port, hostname, function() {
        log.getLogger('nogfw').info('[rest] 服务启动成功, 地址: ' + server.url);
      });
      server.on('error', function(error) {
        log.getLogger('nogfw').fatal('[rest] ' + error);
        log.getLogger('nogfw').fatal('子进程异常终止', true);
        process.exit(1);
      });
