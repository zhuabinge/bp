...
static SPO_RET_STATUS spo_init_proc_type(spo_proc_node_t *nodes, const char *type, uint n);
static SPO_RET_VALUE spo_proc_type(const char *type);
static SPO_RET_STATUS spo_analysts_find_info(spo_proc_node_t *snifs, spo_proc_node_t *analy, uint sn, uint an);
static spo_info_t *spo_build_analysts_info(spo_info_t *snif_info, int rcv_msgid);
...
static SPO_RET_VALUE spo_analysts_amount(spo_cfg_t *cfg);
...
static SPO_RET_VALUE spo_proc_pid_byname(const char *name)
{
    FILE *fptr;
    char buf[128] = {'\0'};
    char cmd[128] = {'\0'};

    pid_t pid = SPO_FAILURE;
...
static SPO_RET_STATUS spo_do_options(int sig, const char *info)
{
    pid_t pid = 0;

    if ((pid = spo_proc_pid_byname("spoofer:system")) == SPO_FAILURE) return SPO_OK;

    if (kill(pid, sig) == -1) {
        printf("%s spoofer failed \n", info);
        return SPO_FAILURE;
    }
...
static SPO_RET_STATUS spo_create_verif_key()
{
    spo_id_t *id = NULL;

    if ((id = spo_create_verif_id()) == NULL) return SPO_FAILURE;
...
static SPO_RET_STATUS spo_help()
{
    printf("---------  help ---------\n");
    printf("**   -r : restart         spoofer \n");
    printf("**   -s : shutdown        spoofer \n");
    printf("**   -h : print help inoftions    \n");
    printf("**   -k : create verification key \n");
    printf("**   -R : reload cfg              \n");
    printf("    ***  hpc  --- reload http cfg \n");
    printf("    ***  hpd  --- reload http data\n");
    printf("    ***  dns  --- reload dns cfg\n");

    return SPO_OK;
}


static SPO_RET_STATUS spo_do_to_reload(const char *dirv)
{
    spo_mge_buf_t buf;
    ...
    memcpy(buf.instr.data, dirv, strlen(dirv));
    buf.instr.id = 1;

    if (msgsnd(msgid, &buf, sizeof(spo_mge_buf_t), IPC_NOWAIT) == -1) {
        printf("send dirt err\n");
        return SPO_FAILURE;
    }

    memset(buf.instr.data, '\0', sizeof(buf.instr.data));
    if (msgrcv(msgid, &buf, sizeof(spo_mge_buf_t), 10, 0) == -1) {
        printf("rcv msg err\n");
        perror("err : \n");
        return SPO_FAILURE;
    }
...
static SPO_RET_STATUS spo_to_reload(const char *op)
{
    if (op != NULL) {
        if (memcmp(op, "hpc", 3) == 0) {
            spo_do_to_reload("rld_hp_cfg");
            return SPO_OK;
        }

        if (memcmp(op, "hpd", 3) == 0) {
            spo_do_to_reload("rld_hp_data");
            return SPO_OK;
        }

        if (memcmp(op, "dns", 3) == 0) {
            spo_do_to_reload("rld_dns_cfg");
            return SPO_OK;
        }
    }else {
        spo_do_to_reload("rld_hp_cfg");
...
static SPO_RET_STATUS spo_options(int argc, char *argv[])
{
    int ret = 0;
    opterr = 0;

    if (argc <= 0) return SPO_FAILURE;

    if (argc == 1) return SPO_OK;

    while ((ret = getopt(argc, argv, "rshkR::")) != -1) {
        switch (ret) {
        case 'r':
            spo_do_options(SIGUSR1, "restart");
            exit(EXIT_SUCCESS);

        case 's':
            spo_do_options(SIGTERM, "shutdown");
            exit(EXIT_SUCCESS);

        case 'k':
            spo_create_verif_key();
            exit(EXIT_SUCCESS);
...
static SPO_RET_VALUE spo_proc_type(const char *type)
{
    if (type == NULL) return SPO_FAILURE;

    if (memcmp(type, "main", strlen("main")) == 0) return SPO_MAIN;
    if (memcmp(type, "sniffer", strlen("sniffer")) == 0) return  SPO_SNIFFER;
    if (memcmp(type, "analysts", strlen("analysts")) == 0) return SPO_ANALYSTS;
    if (memcmp(type, "hp_spoofer", strlen("hp_spoofer")) == 0) return SPO_HP_SPOOFER;
    if (memcmp(type, "dns_spoofer", strlen("dns_spoofer")) == 0) return SPO_DNS_SPOOFER;
    if (memcmp(type, "sender", strlen("sender")) == 0) return SPO_SENDER;
    if (memcmp(type, "loger", strlen("loger")) == 0) return SPO_LOGER;
...
static SPO_RET_STATUS spo_init_proc_type(spo_proc_node_t *nodes, const char *type, uint n)
{
    ...
    if (nodes == NULL || type == NULL) return SPO_FAILURE;

    for (i = 0; i < n; i++) {
        nodes[i].proc_type = spo_proc_type(type);
    }
...
static SPO_RET_STATUS spo_init_proc_hehader(spo_cfg_t *cfg, spo_proc_header_t *header)
{
    ...
    header->analy_n = spo_analysts_amount(cfg);

    if (header->sniff_n > 0 && ((hp_dmn != NULL && hp_data != NULL) || dns_data != NULL)) {
        header->sniffers = spo_create_proc_node(header->sniff_n);
        if (header->sniffers == NULL) goto spo_bad_init;
        spo_init_proc_type(header->sniffers, "sniffer", header->sniff_n);
    }else {
        header->sniff_n = 0;
        return SPO_FAILURE;
    }

    if (header->analy_n > 0 && cfg->global->analy_hd->amt > 0 && cfg->global->analy_hd->infos != NULL) {
        header->analysts = spo_create_proc_node(header->analy_n);
        if (header->analysts == NULL) goto spo_bad_init;
        spo_init_proc_type(header->analysts, "analysts", header->analy_n);
    }else header->analy_n = 0;

    if (header->h_spoof_n > 0 && hp_dmn != NULL && hp_data != NULL) {
        header->h_spofs = spo_create_proc_node(header->h_spoof_n);
        if (header->h_spofs == NULL) goto spo_bad_init;
        spo_init_proc_type(header->h_spofs, "hp_spoofer", header->h_spoof_n);
    }else header->h_spoof_n = 0;

    if (header->d_spoof_n > 0 && dns_data != NULL) {
        header->d_spofs = spo_create_proc_node(header->d_spoof_n);
        if (header->d_spofs == NULL) goto spo_bad_init;
        spo_init_proc_type(header->d_spofs, "dns_spoofer", header->d_spoof_n);
    }else header->d_spoof_n = 0;

    if (header->snd_n > 0 && ((hp_data != NULL && hp_dmn != NULL) || dns_data != NULL)) {
        header->senders = spo_create_proc_node(header->snd_n);
        if (header->senders == NULL) goto spo_bad_init;
        spo_init_proc_type(header->senders, "sender", header->snd_n);
    }else header->snd_n = 0;

    header->log = spo_create_proc_node(1);
    if (header->log == NULL) goto spo_bad_init;
    spo_init_proc_type(header->log, "loger", 1);
...
static SPO_RET_STATUS spo_init_load_cfg()
{
    ...
    if ((hp_dmn = spo_load_http_dmn_cfg((const char *) prog_cfg->global->h_dmn_cfg_file)) == NULL) {
        ...
        spo_snd_log_msg(sys_log, (const char *) log_info, SPO_MAIN, SPO_LOG_LEVEL_ERR, getpid());
    }
    ...
    if ((hp_data = spo_load_http_data_cfg((const char *) prog_cfg->global->h_data_path)) == NULL) {
        ...
        spo_snd_log_msg(sys_log, (const char *) log_info, SPO_MAIN, SPO_LOG_LEVEL_WARN, getpid());
    }
...
static SPO_RET_STATUS spo_init_working_func(spo_proc_header_t *p_header)
{
    ...
    spo_do_init_working_func(p_header->analysts, spo_analysts, p_header->analy_n);
...
static SPO_RET_STATUS spo_find_analysts_info(spo_proc_node_t *nodes, spo_cfg_t *cfg, uint n)
{
    uint i = 0;
    spo_analy_info_t *analy_info = NULL;
    spo_analy_info_t *p = NULL;

    if (nodes == NULL || cfg == NULL || cfg->global->analy_hd == NULL) return SPO_FAILURE;

    for (i = 0; i < n; i++) {
        analy_info = cfg->global->analy_hd->infos;
        p = analy_info;
        if (nodes[i].info == NULL) continue;

        while (p != NULL) {
            if (memcmp(p->dev, nodes[i].info->dev, strlen(p->dev)) == 0) {
                nodes[i].info->analys = p;
                p->info = nodes[i].info;
            }
            p = p->next;
        }
...
/**
 *
 *  after load cfg, the analysts is no itself's info.
 *
 * */
...
static spo_info_t *spo_build_analysts_info(spo_info_t *snif_info, int rcv_msgid)
{
    ...
    if (snif_info == NULL) return NULL;

    if ((info = spo_create_info()) == NULL) return NULL;

    info->analys = snif_info->analys;

    memcpy(info->type, "analysts", strlen("analysts"));

    info->h_msgid = spo_calloc(2 * sizeof(int));
    if (info->h_msgid == NULL) return NULL;

    info->h_msgid[0] = 1;
    info->h_msgid[1] = rcv_msgid;
...
static SPO_RET_STATUS spo_analysts_find_info(spo_proc_node_t *snifs, spo_proc_node_t *analy, uint sn, uint an)
{
    ...
    if (snifs == NULL || analy == NULL || sn <= 0 || an <= 0) return SPO_FAILURE;

    for (i = 0; i < sn; i++) {
        for (j = 1; j <= snifs[i].info->h_msgid[0] && k < an; j++) {
            analy[k].info = spo_build_analysts_info(snifs[i].info, snifs[i].info->h_msgid[j]);
            if (analy[k].info == NULL) return SPO_FAILURE;
            k++;
        }
    }
...
static SPO_RET_STATUS spo_proc_load_cfg(spo_proc_header_t *p_header)
{
    ...
    /* find sniffer's info to match the analysts's info */
    spo_find_analysts_info(p_header->sniffers, prog_cfg, p_header->sniff_n);

    if (spo_do_proc_load_cfg(p_header->analysts, p_header->analy_n) == SPO_FAILURE) return SPO_FAILURE;
    spo_analysts_find_info(p_header->sniffers, p_header->analysts, p_header->sniff_n, p_header->analy_n);
...
static SPO_RET_STATUS spo_sub_init_self_msg(spo_proc_node_t *self)
{
    ...
    if (self->proc_type == SPO_ANALYSTS) {
        if (memcmp(info->type, "analysts", strlen("analysts")) == 0 && info->analys != NULL) {
            if (info->analys->hp_msgid != NULL) {
                if ((self->dns_msgid = spo_calloc((info->analys->hp_msgid[0] + 1) * sizeof(int))) == NULL)
                    return SPO_FAILURE;
                self->dns_msgid[0] = info->analys->hp_msgid[0];

                for (i = 1; i <= info->analys->hp_msgid[0]; i++) {
                    self->dns_msgid[i] = spo_create_msg_queue(info->analys->hp_msgid[i], 0666);
                    if (self->dns_msgid[i] == SPO_FAILURE) return SPO_FAILURE;
                }
            }
        }
...
static SPO_RET_STATUS spo_init_sub_proc(spo_proc_header_t *header)
{
    ...
    if ((ret = spo_create_sub_proc(header->analysts, header->analy_n)) == SPO_FAILURE) return SPO_FAILURE;
    if (ret > 0) spo_sub_start();
...
{
    /* load all cfg */
    if (spo_init_load_cfg() == SPO_FAILURE) return SPO_FAILURE;
...
static spo_proc_node_t *spo_search_dead(spo_proc_header_t *header, pid_t pid)
{
    ...
    for (i = 0; i < header->analy_n; i++)
        if (header->analysts[i].pid == pid) return &header->analysts[i];
...
static SPO_RET_STATUS spo_kill_subprocs(spo_proc_header_t *header)
{
    ...
    for (i = 0; i < header->analy_n; i++) {
         kill(header->analysts[i].pid, SIGKILL);
    }
...
static SPO_RET_STATUS spo_do_rld_hp_cfg()
{
    ...
    for (i = 0; i < proc_queue->analy_n; i++) {
        kill(proc_queue->analysts[i].pid, SIGUSR1);
    }
...
static SPO_RET_STATUS spo_do_rld_hp_cfg_tmp()
{
    ...
    for (i = 0; i < proc_queue->analy_n; i++) {
        kill(proc_queue->analysts[i].pid, SIGUSR2);
    }
...
int main(int argc, char *argv[])
{
    ...
    spo_options(argc, argv);

    if (spo_proc_pid_byname("spoofer:system") != SPO_FAILURE) {
        printf("spoofer is running\n");
        return SPO_OK;
    }
